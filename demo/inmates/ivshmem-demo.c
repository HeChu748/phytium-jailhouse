...
struct ivshmem_dev_data {
    ...
    u32 *rw_section;
    u64 rw_section_sz;
    s32 *in_sections;   /* 改为 s32 */
    s32 *out_section;   /* 改为 s32 */
    u64 out_section_sz;
    u32 *msix_table;
    u32 id;
    int msix_cap;
...
static void irq_handler(unsigned int irq)
{
    ...
    dev.out_section[0]     = (s32)(value * 10);

    printk("\nIVSHMEM: got interrupt %u (#%d)\n", n, irq_counter[n]);
    print_shmem(&dev);
}
...
static void init_device(struct ivshmem_dev_data *d)
{
    ...
    d->in_sections = (s32 *)addr;           /* 改为 s32* */

    addr += d->id * d->out_section_sz;
    d->out_section = (s32 *)addr;           /* 改为 s32* */
